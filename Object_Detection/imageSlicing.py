...
def is_frame_running(capture):
    # Read the next frame from the video capture object
    ret, frame = capture.read()

    # If the frame was successfully read, return True
    if ret:
        return True
    # If the frame was not successfully read, return False
    else:
        return False

while is_frame_running(img):
    height = img.shape[0]
    width = img.shape[1]
