    ...
    while True:
        ...
        if key in [ord('k'), ord('l')]:
            ...
            thresholded = cv.adaptiveThreshold(
                feed, 255, threshold_type, cv.THRESH_BINARY, block_size, C)
            thresholded = cv.bitwise_not(thresholded)
            _, thresholded = cv.threshold(
                thresholded, 128, 255, cv.THRESH_BINARY)
            ...
            contours, _ = cv.findContours(
                feed, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
