...
#-------------------------------------------------------------------------------------#
#Import
partImg = cv.imread('AutoMask\mask_pics\STANDARD5.jpg')
noPartImg = cv.imread('AutoMask\mask_pics\\NONE5.jpg')
...
for i in range(partImg.shape[0]):
    for j in range(partImg.shape[1]):
        if (partImg[i, j, 0] - 5) >= noPartImg[i, j, 0]:
            mask[i, j] = 255

subtract1 = cv.cvtColor(subtract1, cv.COLOR_BGR2GRAY)

#Subtracting the two images to find the part area
noPartImgMasked = cv.bitwise_and(noPartImg,noPartImg,mask=cv.bitwise_not(mask))
...
#Passing
# subtract1 = cv.bitwise_and(partImg, partImg, mask = result)
# subtract2 = cv.bitwise_and(subtract2, subtract2, mask = result)
# feed = subtract1-subtract2
result = subtract2
